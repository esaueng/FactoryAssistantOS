...
the deeper P3 changes and now includes native plant navigation, the native
read-only `fa-machine-card` tile, `fa-andon-view` board, and
`factory-wallboard-kiosk` component. Native dashboard wiring is implemented in
the shipped YAML templates with `custom:fa-machine-card`,
`custom:fa-andon-view`, and `custom:factory-wallboard-kiosk`; the area
dashboard generator produces `area-dashboards.example.yaml` from the shipped
line/cell model with `custom:fa-machine-card` and `custom:fa-andon-view`.
Industrial terminology and full onboarding integration still remain while the
fork carries the visible product identity (`docs/BRANDING.md`). It stays
...
- Appliance on the plant LAN; discovery via mDNS (`factory-assistant.local`);
  DHCP default, static IP supported via NetworkManager (CLI plugin or UI). The
  OS ships `network/network_identity_contract.yaml` to keep hostname, mDNS,
  local web URL, Core display name, and onboarding guidance aligned across forks.
...
| P3 | Industrial onboarding, config seeding, OPC UA bridge add-on, frontend fork implementing the factory UI (`docs/UI_DESIGN.md`) | partial: seed config, Plant overview default dashboard, examples, site/line/cell onboarding scaffold, area dashboard generator, industrial onboarding wizard contract, frontend experience contract, Supervisor seed contract, network identity contract, industrial add-on catalog, published industrial add-on manifests, wallboard, theme, OS-level network/time posture helper, and local-first Core defaults exist; cloud/analytics defaults are off in the shipped template; frontend fork has visible rebrand/About/local-first onboarding bridge, native plant navigation, a native read-only `fa-machine-card`, a native read-only `fa-andon-view`, and a native read-only `factory-wallboard-kiosk`; native dashboard cards are wired with `custom:fa-machine-card`, `custom:fa-andon-view`, and `custom:factory-wallboard-kiosk`; P3 backlog is full industrial onboarding wizard integration and terminology polish |
