...
Each entry includes an upstream-compatible add-on manifest contract: slug,
image name, supported architecture, startup/boot policy, and the required
non-host, non-privileged runtime posture. The option schemas are intentionally
small and site-editable so the downstream `factory-assistant-addons`
repository can implement the same read-only defaults without inventing product
behavior.
